...
import {
  ...
  ChevronRight, ChevronLeft, Check, Star, GripVertical,
} from "lucide-react";
...
export default function AddProduct() {
  ...
  // Images: ordered list — index 0 = main/background image
  ...
  // FIX: Generate SKU once for the summary, stable across renders
  const [previewSku, setPreviewSku] = useState("");
  useEffect(() => {
    if (!sku) setPreviewSku(generateSKU(name || "PROD", category));
  }, [name, category, sku]);

  // FIX: Cleanup object URLs on unmount to prevent memory leaks
  useEffect(() => {
    return () => {
      images.forEach((img) => {
  ...
  const handleFileUpload = (files: FileList | null) => {
    ...
    toAdd.forEach((file) => {
      ...
      // FIX: Use stable unique id as key instead of index
  ...
  // ── Submit ─────────────────────────────────────────
  // FIX: Accept status as direct parameter to avoid race condition with setStatus
  const handleSubmit = async (submitStatus: "Active" | "Draft" = "Active") => {
    ...
    // FIX: Warn if all sizes have 0 stock (still allow submit but warn)
    if (totalStock === 0) {
      toast.warning("All sizes have 0 stock — product will be marked as out of stock");
    }
    ...
    try {
      ...
      await addProduct({
        ...
        featured,
        status: submitStatus,  // FIX: use the parameter directly, not state
        weight: weight ? parseFloat(weight) : undefined,
      });
      toast.success(submitStatus === "Draft" ? "Saved as draft!" : "Product published successfully!");
      navigate("/admin/products");
    } catch (err: any) {
      console.error(err);
      toast.error(err.response?.data?.error || err.response?.data?.message || err.message || "Failed to save product");
    } finally {
      setLoading(false);
    }
  };

  // FIX: Confirm before leaving with unsaved changes
  const handleCancel = () => {
    const hasData = name || price || images.length > 0 || sizeStocks.length > 0;
    if (hasData) {
        ...
        >
          ...
          {step === 1 && (
            <>
              ...
              <p className="text-sm text-muted-foreground">
                Drag to reorder images • Click ⭐ to set the Home page background image
                {images.length > 0 && (
              ...
              {images.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {/* FIX: Use stable id as key instead of index */}
                    {images.map((img, i) => (
                      <div
                        key={img.id}
                        draggable
                      ...
                      >
                        ...
                        {/* Main badge */}
                        {i === 0 && (
                          <span className="absolute top-2 left-2 px-2 py-0.5 bg-[#dc143c] text-white text-[10px] font-bold rounded-full">
                            Main
                          </span>
                        )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    step={0.01}
                  />
                  {/* FIX: Show validation error for price <= 0 */}
                  {price && basePrice <= 0 && (
          ...
          {step === 3 && (
            <>
              ...
              <p className="text-sm text-muted-foreground">
                Select available sizes and set the stock quantity for each
              </p>
                    ...
                    {sizeStocks.map(({ size, stock }) => (
                      <div key={size} className="bg-white/5 rounded-xl p-3 border border-white/10">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => updateSizeStock(size, stock - 1)}
                            className="w-7 h-7 rounded-full bg-secondary hover:bg-secondary/70 text-sm font-bold transition-colors flex items-center justify-center"
                          >−</button>
                          <input
                            type="number"
                            value={stock}
              ...
              {sizeStocks.length === 0 && (
                <div className="text-center py-8 text-muted-foreground text-sm">
                  Select at least one size from above
                </div>
              )}
          ...
          {step === 4 && (
            <>
              ...
              <div className="bg-black/30 rounded-xl p-4 space-y-2 text-sm">
                <h3 className="font-bold mb-3 text-white">Product Summary</h3>
                ...
                {/* FIX: Use stable previewSku, not regenerated every render */}
      ...
      <div className="flex items-center justify-between">
        ...
        {step < STEPS.length - 1 ? (
          ...
          <div className="flex gap-3">
            {/* FIX: Pass "Draft" directly as argument — no setStatus race condition */}
